...
# LSTM params (dict or list of dicts)
MODEL_NAME = "lstm"
MODEL_PARAMS = {
    "lstm_units": 32,
    "dropout": 0.1,
...
# Outputs
OUTPUT_SUMMARY_PATH = "backtest_grid_summary_lstm.xlsx"
SAVE_WEIGHTS = False
WEIGHTS_OUTPUT_DIR = "backtest_weights_grid_lstm"
...
def compute_oos_mse_for_omega(df_pred, target_return, train_end_date):
    ...
    tmp = df_pred.loc[mask, [target_return, "pred_return"]].dropna()
    if len(tmp) < 2:
        raise ValueError("Not enough OOS samples to compute MSE for Omega.")
    y_true = tmp[target_return].values
    y_pred = tmp["pred_return"].values
...
def build_lstm(input_shape, lstm_units, dropout, learning_rate):
...
def run_lstm_full_eval(df, embedding_cols, target_return, train_end_date, params):
    df_tmp = df.dropna(subset=[target_return]).copy()
    df_tmp["date"] = pd.to_datetime(df_tmp["date"])
    tf.keras.backend.clear_session()
    train_mask = df_tmp["date"] <= pd.to_datetime(train_end_date)
    test_mask = df_tmp["date"] > pd.to_datetime(train_end_date)
    ...
    cb = []
    if params.get("patience", 0) and params["patience"] > 0:
    ...
    validation_split = 0.1 if len(y_train) >= 50 else 0.0
    model.fit(
        X_train_seq,
        y_train,
    ...
    y_pred_train = model.predict(X_train_seq, verbose=0).reshape(-1)
    y_pred_test = model.predict(X_test_seq, verbose=0).reshape(-1)
...
def run_lstm_and_predict(df, embedding_cols, target_return, train_end_date, params):
    df_tmp = df.dropna(subset=[target_return]).copy()
    df_tmp["date"] = pd.to_datetime(df_tmp["date"])
    tf.keras.backend.clear_session()
    ...
    df_tmp.loc[train_mask, "pred_return"] = model.predict(X_train_seq, verbose=0).reshape(-1)
    df_tmp.loc[test_mask, "pred_return"] = model.predict(X_test_seq, verbose=0).reshape(-1)
...
def main():
    ...
    model_params_list = MODEL_PARAMS if isinstance(MODEL_PARAMS, (list, tuple)) else [MODEL_PARAMS]
    ...
    for model_params in model_params_list:
        ...
        df_pred, _ = run_lstm_and_predict(
            df=df_agg,
            embedding_cols=embedding_cols,
        ...
        for col in BASE_UNIVERSE_RETURN_COLS:
            ...
            for min_cnt in MIN_REPORT_COUNTS:
                ...
                for window in windows:
                    ...
                    for view_date in tqdm(view_dates, desc=f"BT {MODEL_NAME} w={window} {col}/{min_cnt}"):
                        try:
