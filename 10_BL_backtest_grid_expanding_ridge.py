...
# Outputs
OUTPUT_SUMMARY_PATH = "backtest_grid_summary_expanding.xlsx"
SAVE_WEIGHTS = False
...
def run_ridge_and_predict_expanding(df, embedding_cols, target_return, train_end_date, alpha=10.0):
    df_tmp = df.dropna(subset=[target_return]).copy()
    ...
    model = None
    for d in tqdm(dates, desc="Ridge expanding"):
        if d <= train_end_date:
            continue
        train_mask = df_tmp["date"] < d
        if train_mask.sum() < 2:
            continue
        X_train = df_tmp.loc[train_mask, embedding_cols].values
        y_train = df_tmp.loc[train_mask, target_return].values
...
def main():
    ...
    for alpha in ridge_alphas:
        ...
        df_pred, _ = run_ridge_and_predict_expanding(
            df=df_agg,
            embedding_cols=embedding_cols,
