...
RIDGE_TRAIN_WINDOW = 252  # rolling window size in unique dates
...
# Outputs
OUTPUT_SUMMARY_PATH = "backtest_grid_summary_rollling.xlsx"
SAVE_WEIGHTS = False
...
def run_ridge_and_predict_rolling(
    df,
    embedding_cols,
    target_return,
    train_end_date,
    ...
    df_tmp["pred_return"] = np.nan

    dates = np.sort(df_tmp["date"].unique())
    train_end_date = pd.to_datetime(train_end_date)

    model = None
    for idx, d in enumerate(tqdm(dates, desc="Ridge rolling")):
        if d <= train_end_date:
            continue
        start_idx = max(0, idx - int(train_window))
        train_dates = dates[start_idx:idx]
        ...
        X_train = df_tmp.loc[train_mask, embedding_cols].values
        y_train = df_tmp.loc[train_mask, target_return].values
        X_pred = df_tmp.loc[df_tmp["date"] == d, embedding_cols].values
        if len(X_pred) == 0:
            continue
        ...
        model.fit(X_train, y_train)
        df_tmp.loc[df_tmp["date"] == d, "pred_return"] = model.predict(X_pred)
...
def main():
    ...
    for alpha in ridge_alphas:
        ...
        df_pred, _ = run_ridge_and_predict_rolling(
            df=df_agg,
            embedding_cols=embedding_cols,
