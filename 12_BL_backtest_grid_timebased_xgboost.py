...
# XGBoost params (dict or list of dicts)
MODEL_NAME = "xgboost"
MODEL_PARAMS = {
    "n_estimators": 300,
    "max_depth": 4,
...
# Grid parameters
BASE_UNIVERSE_RETURN_COLS = ["log_return_1"]
MIN_REPORT_COUNTS = [10]
...
WINDOW = [504]  # scalar or list (e.g. [252, 504])
MIN_COVERAGE = 0.90
RISK_AVERSION = 2.5
TAU = 0.025  # None => 1/n
...
# Outputs
OUTPUT_SUMMARY_PATH = "backtest_grid_summary_xgboost.xlsx"
SAVE_WEIGHTS = False
WEIGHTS_OUTPUT_DIR = "backtest_weights_grid_xgboost"
...
def run_xgb_full_eval(df, embedding_cols, target_return, train_end_date, params):
...
def run_xgb_and_predict(df, embedding_cols, target_return, train_end_date, params):
    df_tmp = df.dropna(subset=[target_return]).copy()
    df_tmp["date"] = pd.to_datetime(df_tmp["date"])
